...
function renderClassAffairs(code) { const root=list("class-affairs-list"); const datasets=classAffairsTemplates.length?classAffairsTemplates:(classAffairsData?.students?[DEFAULT_CLASS_AFFAIRS_DATASET]:[]); if(selectedClassAffairsDatasetId&&selectedClassAffairsDatasetId!==CLASS_TIMETABLE_ID&&!datasets.some((dataset)=>dataset.id===selectedClassAffairsDatasetId))selectedClassAffairsDatasetId=""; if(selectedClassAffairsDatasetId===CLASS_TIMETABLE_ID){renderClassTimetable(code);return;} if(!selectedClassAffairsDatasetId){root.innerHTML=`<section class="class-affairs-summary"><p class="card-date">${escapeHtml(code)} 班</p><h3>班務資料</h3><div class="class-affairs-summary-grid"><article class="card class-timetable-summary"><h4>班級課表</h4><p>第一學期</p><button data-class-affairs-timetable>查看課表</button></article>${datasets.map((dataset)=>{const count=(classAffairsGroups(classAffairsData||{})[dataset.id]?.records||[]).length;return `<article class="card"><h4>${escapeHtml(dataset.name)}</h4><p><strong>${count}</strong> 筆資料</p><button data-class-affairs-dataset="${escapeHtml(dataset.id)}">查看資料</button></article>`;}).join("")}</div></section>`;root.querySelector("[data-class-affairs-timetable]").onclick=()=>{selectedClassAffairsDatasetId=CLASS_TIMETABLE_ID;renderClassAffairs(code);};root.querySelectorAll("[data-class-affairs-dataset]").forEach((button)=>button.onclick=()=>{selectedClassAffairsDatasetId=button.dataset.classAffairsDataset;renderClassAffairs(code);});return;} const dataset=datasets.find((item)=>item.id===selectedClassAffairsDatasetId); const records=sortClassAffairsRecords(classAffairsGroups(classAffairsData||{})[dataset.id]?.records||[]); root.innerHTML=`<article class="card class-affairs-card"><button class="secondary class-affairs-back" type="button">‹ 返回班務資料</button><p class="card-date">${escapeHtml(code)} 班</p><h3>${escapeHtml(dataset.name)}</h3><p class="class-affairs-count">共 ${records.length} 筆資料</p>${records.length?`<div class="class-affairs-records">${records.map((record,index)=>`<article><strong>${index+1}</strong><dl>${dataset.fields.map((field)=>`<div><dt>${escapeHtml(field)}</dt><dd>${escapeHtml(record.values?.[field]||"－")}</dd></div>`).join("")}</dl></article>`).join("")}</div>`:"<p>目前尚未建立資料。</p>"}</article>`;root.querySelector(".class-affairs-back").onclick=()=>{selectedClassAffairsDatasetId="";renderClassAffairs(code);}; }
