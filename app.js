...
if ("serviceWorker" in navigator) window.addEventListener("load", () => navigator.serviceWorker.register("./service-worker.js?v=93").then((registration) => registration.update()));
...
const PERSONAL_TIMETABLE_ID = "personal_timetable";
...
function classAffairsGroups(data = {}) { return data.groups || (data.students ? { roster: { records: data.students.map((student,index)=>({id:student.id||`legacy-${index}`,values:{"座號":String(student.seatNumber||""),"姓名":student.name||"","OpenID 帳號":student.openId||""}})) } } : {}); }
...
function timetablePeriods(data = {}) { const source = data?.timetable?.periods || {}; return Object.fromEntries(TIMETABLE_PERIODS.map((period) => [period, Object.fromEntries(TIMETABLE_DAYS.map((day) => [day.key, String(source[period]?.[day.key] || "")]))])); }
function timetableShortCourse(value) { const name = String(value || "").replace(/\s+/g, "").replace(/[－—–]/g, "-"); return name ? COURSE_ABBREVIATIONS[name] || Array.from(name).slice(0, 2).join("") : "－"; }
function renderClassTimetable(code) { const root = list("class-affairs-list"); const schedule = timetablePeriods(classAffairsData || {}); root.innerHTML = `<article class="card class-affairs-card class-timetable-card"><button class="secondary class-affairs-back" type="button">‹ 返回班務資料</button><p class="card-date">${escapeHtml(code)} 班</p><h3>班級課表</h3><p class="class-affairs-count">課程名稱以兩字顯示</p><div class="class-timetable-wrap"><table class="class-timetable-table"><thead><tr><th>節次</th>${TIMETABLE_DAYS.map((day) => `<th>${day.label}</th>`).join("")}</tr></thead><tbody>${TIMETABLE_PERIODS.map((period) => `<tr><th>${period}</th>${TIMETABLE_DAYS.map((day) => { const course = schedule[period][day.key]; return `<td title="${escapeHtml(course)}">${escapeHtml(timetableShortCourse(course))}</td>`; }).join("")}</tr>`).join("")}</tbody></table></div></article>`; root.querySelector(".class-affairs-back").onclick = () => { selectedClassAffairsDatasetId = ""; renderClassAffairs(code); }; }
...
function renderClassAffairs(code) {
  const root = list("class-affairs-list");
  const datasets = classAffairsTemplates.length ? classAffairsTemplates : (classAffairsData?.students ? [DEFAULT_CLASS_AFFAIRS_DATASET] : []);
  if (selectedClassAffairsDatasetId && selectedClassAffairsDatasetId !== CLASS_TIMETABLE_ID && !datasets.some((dataset) => dataset.id === selectedClassAffairsDatasetId)) selectedClassAffairsDatasetId = "";
  if (selectedClassAffairsDatasetId === CLASS_TIMETABLE_ID) { renderClassTimetable(code); return; }
  const personalDataset = datasets.find((dataset) => dataset.id === PERSONAL_TIMETABLE_ID || dataset.name === "個人課表");
  if (selectedClassAffairsDatasetId && personalDataset && selectedClassAffairsDatasetId === personalDataset.id) { renderPersonalTimetable(code, personalDataset); return; }
  if (!selectedClassAffairsDatasetId) {
    const personalCard = personalDataset ? `<article class="card class-timetable-summary"><h4>個人課表</h4><p>第一學期</p><button data-class-affairs-dataset="${escapeHtml(personalDataset.id)}">查看課表</button></article>` : "";
    const otherCards = datasets.filter((dataset) => dataset.id !== personalDataset?.id).map((dataset) => { const count = (classAffairsGroups(classAffairsData || {})[dataset.id]?.records || []).length; return `<article class="card"><h4>${escapeHtml(dataset.name)}</h4><p><strong>${count}</strong> 筆資料</p><button data-class-affairs-dataset="${escapeHtml(dataset.id)}">查看資料</button></article>`; }).join("");
    root.innerHTML = `<section class="class-affairs-summary"><p class="card-date">${escapeHtml(code)} 班</p><h3>班務資料</h3><div class="class-affairs-summary-grid">${personalCard}<article class="card class-timetable-summary"><h4>班級課表</h4><p>第一學期</p><button data-class-affairs-timetable>查看課表</button></article>${otherCards}</div></section>`;
    root.querySelector("[data-class-affairs-timetable]").onclick = () => { selectedClassAffairsDatasetId = CLASS_TIMETABLE_ID; renderClassAffairs(code); };
    root.querySelectorAll("[data-class-affairs-dataset]").forEach((button) => button.onclick = () => { selectedClassAffairsDatasetId = button.dataset.classAffairsDataset; renderClassAffairs(code); });
    return;
  }
  const dataset = datasets.find((item) => item.id === selectedClassAffairsDatasetId);
  const records = sortClassAffairsRecords(classAffairsGroups(classAffairsData || {})[dataset.id]?.records || []);
  root.innerHTML = `<article class="card class-affairs-card"><button class="secondary class-affairs-back" type="button">‹ 返回班務資料</button><p class="card-date">${escapeHtml(code)} 班</p><h3>${escapeHtml(dataset.name)}</h3><p class="class-affairs-count">共 ${records.length} 筆資料</p>${records.length ? `<div class="class-affairs-records">${records.map((record, index) => `<article><strong>${index + 1}</strong><dl>${dataset.fields.map((field) => `<div><dt>${escapeHtml(field)}</dt><dd>${escapeHtml(record.values?.[field] || "－")}</dd></div>`).join("")}</dl></article>`).join("")}</div>` : "<p>目前尚未建立資料。</p>"}</article>`;
  root.querySelector(".class-affairs-back").onclick = () => { selectedClassAffairsDatasetId = ""; renderClassAffairs(code); };
}
