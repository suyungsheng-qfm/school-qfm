...
function renderClassAffairs(data, code) { const root=list("class-affairs-list"); const students=[...(data?.students||[])].sort((a,b)=>Number(a.seatNumber)-Number(b.seatNumber)); if(!students.length){root.innerHTML=`<article class="card"><h3>${escapeHtml(code)} 班班務</h3><p>目前尚未建立名冊資料。</p></article>`;return;} root.innerHTML=`<article class="card class-affairs-card"><p class="card-date">${escapeHtml(code)} 班</p><h3>班級名冊</h3><div class="class-affairs-head"><span>座號</span><span>姓名</span><span>OpenID 帳號</span></div><ul class="class-affairs-roster">${students.map((student)=>`<li><strong>${escapeHtml(student.seatNumber)}</strong><span>${escapeHtml(student.name)}</span><small>${escapeHtml(student.openId)}</small></li>`).join("")}</ul></article>`; }
