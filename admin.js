...
const PERSONAL_TIMETABLE_DATASET = { id: "personal_timetable", name: "個人課表", fields: ["節次", "星期一", "星期二", "星期三", "星期四", "星期五"] };
...
async function loadClassAffairsTemplates() {
  ...
  const savedTemplates = Array.isArray(config.datasets) ? config.datasets : [DEFAULT_CLASS_AFFAIRS_DATASET];
  const existingPersonal = savedTemplates.find((dataset) => normalizeDatasetName(dataset.name) === normalizeDatasetName(PERSONAL_TIMETABLE_DATASET.name));
  classAffairsTemplates = existingPersonal
    ? [existingPersonal, ...savedTemplates.filter((dataset) => dataset.id !== existingPersonal.id)]
    : [PERSONAL_TIMETABLE_DATASET, ...savedTemplates];
  ...
  if (!snapshot.exists() || !existingPersonal || savedTemplates[0]?.id !== classAffairsTemplates[0]?.id) await setDoc(reference, { datasets: classAffairsTemplates, updatedAt: serverTimestamp() }, { merge: true });
...
async function renderClassAffairsAdmin() {
  ...
  try {
    ...
    if (classAffairsView === "overview") { selectedClassAffairsDatasetId = ""; overview.hidden = false; config.hidden = true; recordsPanel.hidden = true; timetablePanel.hidden = true; if (statistics) statistics.hidden = true; overview.innerHTML = `<h3>班務資料</h3><p class="field-note">選擇已建立的資料組、班級課表，或建立新的資料組。</p><div class="class-affairs-labels">${classAffairsTemplates.map((dataset) => `<button data-open-class-dataset="${dataset.id}">${escapeHtml(dataset.name)}</button>`).join("")}<button data-open-class-timetable="true">班級課表</button><button class="class-affairs-add" data-new-class-dataset="true">＋ 新增資料組</button></div>`; overview.querySelectorAll("[data-open-class-dataset]").forEach((button) => { button.onclick = () => { selectedClassAffairsDatasetId = button.dataset.openClassDataset; classAffairsView = "detail"; resetClassAffairsDatasetForm(); renderClassAffairsAdmin(); }; }); overview.querySelector("[data-open-class-timetable]").onclick = () => { selectedTimetableClassCode = code; classAffairsView = "timetable"; renderClassAffairsAdmin(); }; overview.querySelector("[data-new-class-dataset]").onclick = () => { selectedClassAffairsDatasetId = ""; classAffairsView = "new"; resetClassAffairsDatasetForm(); renderClassAffairsAdmin(); }; return; }
